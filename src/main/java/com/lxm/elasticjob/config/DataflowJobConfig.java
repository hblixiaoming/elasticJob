...
import com.lxm.elasticjob.job.DataFlowJobImpl;
...
public class DataflowJobConfig {
    ...
    public DataflowJob dataflowJob() {
        return new DataFlowJobImpl();
    }

    @Bean(initMethod = "init")
    public JobScheduler dataflowJobScheduler(final DataflowJob dataflowJob, @Value("${jobs.dataflowJob.cron}") final String cron, @Value("${jobs.dataflowJob.shardingTotalCount}") final int shardingTotalCount,
                                             @Value("${jobs.dataflowJob.shardingItemParameters}") final String shardingItemParameters) {
